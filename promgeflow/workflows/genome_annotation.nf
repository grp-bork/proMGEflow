...
params.gene_caller_batch_size = params.prodigal_batch_size

def gene_caller_batch_size = (params.gene_caller_batch_size >= 1) ? params.gene_caller_batch_size : 1
...
workflow genome_annotation {
	...
	main:
	    ...
	    // if (params.gene_caller_batch_size != null && params.gene_caller_batch_size > 1) {

		def batch_id = 0
		gene_caller_input_ch = genome_data_ch
			.map { speci, genome_id, genome_fasta -> genome_fasta }					
			.buffer(size: gene_caller_batch_size, remainder: true)
		...
		gene_caller_input_ch.dump(pretty: true, tag: "gene_caller_input_ch")

		call_genes(gene_caller_input_ch, gene_caller_batch_size)			

		call_genes.out.annotations.dump(pretty: true, tag: "bp_annotations_ch")

		annotations_ch = call_genes.out.annotations
			.flatten()
			.map { annotation_file -> 
		...
		gene_caller_output_ch = genomes_ch
